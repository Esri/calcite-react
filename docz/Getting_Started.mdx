...
const App = () => {
  return(
    <BoxShadowButton>My Own Custom Button</BoxShadowButton>
  )
}
...
## Theming
Some minor theme editing is possible with calcite-react by extending the `CalciteTheme` config before passing it into the `<CalciteThemeProvider />`. The default theme can be found on [github](https://github.com/ArcGIS/calcite-react/blob/develop/src/CalciteThemeProvider/CalciteTheme.js).
```jsx
import React from 'react';
import ReactDOM from 'react-dom';
