    ...
    const Example = () => {
      ...
      <>
        <div style={{display:"flex"}}>
          {<Button onClick={() => addAccount()} style={{flex:1, margin: "1em"}}> Add Account </Button>}
          {<Button onClick={() => addAccount(options2)} style={{flex:1, margin: "1em"}}> Add Account Auto-Redirects </Button>}
        </div>
