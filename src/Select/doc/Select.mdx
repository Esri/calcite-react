...
`Spacebar`, but not the `Enter` key. The `Enter` key is reserved for form
submissions. When using a filterable Select, the menu can be opened using the
`Down Arrow` key. A Select can be closed without altering its value using the
`Escape` key.
